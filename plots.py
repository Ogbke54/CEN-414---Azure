...
def dispersion_plots(df):
    ...
    def create_plot(fn, label):
        ...
        for year in range(2000,2015):
            ls.append(fn(df[df['Year'] == year]['Observation Value']))
                
        plt.plot(range(2000,2015), ls, label=label)
        plt.title('Population connected to wastewater treatment.')
        plt.ylabel('Value')
        plt.xlabel('Year')
...
def box_plots(df):
    ...
    for idx, year in enumerate(range(2000,2015)):
        sns.boxplot(ax = axes[idx], y = df[df['Year'] == year]['Value'])
        axes[idx].set_title(year)
